...
@section('title', 'Tambah Barang IKC')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        {{-- Menggunakan variabel $lokasi yang dikirim dari IkcController --}}
        <h1 class="h3 text-gray-800">Tambah Barang Inventaris Baru ({{ ucfirst($lokasi) }})</h1>
        <a href="{{ route('cikalang.ikc.index') }}" class="btn btn-secondary btn-sm">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Formulir Data Barang IKC</h6>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('cikalang.ikc.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="rkc_id">Lokasi Ruangan (RKC) <span class="text-danger">*</span></label>
                            <select class="form-control @error('rkc_id') is-invalid @enderror" id="rkc_id" name="rkc_id" required>
                                <option value="">-- Pilih Ruangan --</option>
                                @forelse($rkcs as $rkc)
                                    <option value="{{ $rkc->id }}" {{ old('rkc_id') == $rkc->id ? 'selected' : '' }}>{{ $rkc->name }}</option>
                                @empty
                                    <option value="" disabled>Belum ada data RKC. Silakan tambah data ruangan dulu.</option>
                                @endforelse
                            </select>
                            @error('rkc_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('cikalang.ikc.index') }}" class="btn btn-secondary mr-2">Batal</a>
