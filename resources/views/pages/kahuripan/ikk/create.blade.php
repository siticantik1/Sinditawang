...
@section('title', 'Tambah Barang IKK')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        {{-- Menggunakan variabel $lokasi yang dikirim dari IkkController --}}
        <h1 class="h3 text-gray-800">Tambah Barang Inventaris Baru ({{ ucfirst($lokasi) }})</h1>
        <a href="{{ route('kahuripan.ikk.index') }}" class="btn btn-secondary btn-sm">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Formulir Data Barang IKK</h6>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('kahuripan.ikk.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="rkk_id">Lokasi Ruangan (RKK) <span class="text-danger">*</span></label>
                            <select class="form-control @error('rkk_id') is-invalid @enderror" id="rkk_id" name="rkk_id" required>
                                <option value="">-- Pilih Ruangan --</option>
                                @forelse($rkks as $rkk)
                                    <option value="{{ $rkk->id }}" {{ old('rkk_id') == $rkk->id ? 'selected' : '' }}>{{ $rkk->name }}</option>
                                @empty
                                    <option value="" disabled>Belum ada data RKK. Silakan tambah data ruangan dulu.</option>
                                @endforelse
                            </select>
                            @error('rkk_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('kahuripan.ikk.index') }}" class="btn btn-secondary mr-2">Batal</a>
