...
@section('title', 'Tambah Barang IKT')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        {{-- Menggunakan variabel $lokasi yang dikirim dari IktController --}}
        <h1 class="h3 text-gray-800">Tambah Barang Inventaris Baru ({{ ucfirst($lokasi) }})</h1>
        <a href="{{ route('tawangsari.ikt.index') }}" class="btn btn-secondary btn-sm">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Formulir Data Barang IKT</h6>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('tawangsari.ikt.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="rkt_id">Lokasi Ruangan (RKT) <span class="text-danger">*</span></label>
                            <select class="form-control @error('rkt_id') is-invalid @enderror" id="rkt_id" name="rkt_id" required>
                                <option value="">-- Pilih Ruangan --</option>
                                @forelse($rkts as $rkt)
                                    <option value="{{ $rkt->id }}" {{ old('rkt_id') == $rkt->id ? 'selected' : '' }}>{{ $rkt->name }}</option>
                                @empty
                                    <option value="" disabled>Belum ada data RKT. Silakan tambah data ruangan dulu.</option>
                                @endforelse
                            </select>
                            @error('rkt_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('tawangsari.ikt.index') }}" class="btn btn-secondary mr-2">Batal</a>
